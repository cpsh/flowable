...
基于消息传递通信的分布式系统，不可避免会发生以下错误：进程可能缓慢、被杀死或重启，消息可能会延迟、丢失、重复，在基础Paxos场景中，先不考虑可能出现消息篡改（拜占庭错误）。Paxos算法解决的问题是在一个可能发生上述异常的分布式系统中如何就某个值达成一致，保证不论发生以上任意一种异常都不会影响一致性
Paxos算法 存在三种角色
1.Proposer 提议者 发出提案 proposal
2.Accepter 接受者 可以接受或拒绝提案
3.Leaner 学习者 学习被选定的提案，当提案超过半数的Accepter接受后被批准
更精确的描述
1.决议 proposal只有在被proposer提出后才会被批准
2.一次Paxos算法的执行实例中 只批准一个proposal
3.learner只能获取批准后的proposal
Zookeeper的选举算法有两种，一种是基于basic paxos(Google Chubby采用)实现的，另一种是基于fast paxos(zookeeper采用)算法。默认选举方式是fast paxos，3.4.0之后只保留了FastLeaderElection算法
Zookeeper核心是原子广播，这个机制保证各个Server之间的同步，实现这个机制的协议是ZAB协议(Zookeeper Atomic Broadcast),有两种模式
1.崩溃回复模式（选主）
2.原子广播模式（同步）
1.当服务启动或领导者崩溃后，ZAB进入回复模式，当领导者会选举出来，且大多数Server完成与Leader之间的状态同步后，恢复模式就结束。状态同步保证了leader和follower之间具有相同的系统状态
2.当Zookeeper集群选举出leader并同步完成然后退出恢复模式之后，就进入原子广播模式，所有写请求被转发到leader再由leader将更新propsal广播给follower
为了保证事物的顺序一致性，Zookeeper采用了递增的事务id号（zxid）来标识事务，所有提议（proposal）都在被提出时打上zxid。
Basic Paxos算法流程
...
Zookeeper选主机制
服务器ID
在配置文件中配置项对应的dataDir所指目录下的myid文件中的内容
在配置文件中配置server.<myid>=host:tickPort:electionPort[:ObServer]
编号越大在选主算法中权重越大
数据ID
服务器中存放的最大数据ID
值越大说明数据越新，在选主算法中权重越大
逻辑时钟
...
　　　　3、数据 version 相同的情况下，server id 大的胜出
 ...
 根据这个规则选出 leader。
 ------ 上面可能有误 实际伪集群 第二个服务器启动时就被选为leader而不是第三个
 ------- 以下的说法和上述不同 应该是下面对
